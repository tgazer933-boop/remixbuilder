...
- **Symlinks become plain files on the Windows leg.** Linux/macOS preserve symlinks; Git Bash tar materializes them as regular copies (content intact, `test -L` false). Builds that depend on symlink semantics behave differently on Windows (measured: 3 symlinks → 3 extra regular files on a 10561-file tree).
